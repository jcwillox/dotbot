...
GITHUB_DOWNLOAD=https://github.com/jcwillox/dotbot/releases/download
...
usage() {
	this="$1"
	cat <<EOF
${this}: download dotbot and optionally run dotbot

Usage: ${this} [-b bindir] [-d] [-t tag] [dotbot-args]
  -b sets the installation directory, default is ${BINDIR}.
  -d enables debug logging.
  -t sets the tag, default is ${TAG}.
If dotbot-args are given, after install dotbot is executed with dotbot-args.
EOF
	exit 2
}
...
main() {
	...
	# download tarball
	NAME="dotbot_${VERSION}_${GOOS}_${GOARCH}"
	TARBALL="${NAME}.${FORMAT}"
...
real_tag() {
	...
	release_url="https://github.com/jcwillox/dotbot/releases/${tag}"
